      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'docker tag nodeapp-dev:trunk rajie/nodeapp-dev:latest'
            sh 'docker push rajie/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk rajie/nodeapp-prod:latest'
                            sh 'docker push rajie/nodeapp-prod:latest'
                            sh 'docker save rajie/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
